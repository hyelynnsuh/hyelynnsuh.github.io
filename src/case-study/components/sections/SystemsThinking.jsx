							...
							{points.map((point, index) => (
								<Reveal key={point.title} delay={index * 0.04} className="cs-architecture__point py-5">
									<div className="cs-grid-12 gap-y-2">
										<p className="cs-kicker col-span-4 sm:col-span-2 lg:col-span-2">{point.label}</p>
										<h3 className="cs-subhead col-span-4 sm:col-span-8 lg:col-span-3">{point.title}</h3>
										<p className="cs-body col-span-4 sm:col-span-8 lg:col-span-7">{point.body}</p>
									</div>
								</Reveal>
							))}
						</div>
